...
import { useDeleteTicket, useGetTickets } from "@/app/hooks/useSupport";
...
export default function TicketsTable({ dateRange }: TicketsTableProps) {
    ...
    const limit = 10;
    const { data, isLoading, isFetchingNextPage, hasNextPage, fetchNextPage } = useGetTickets(businessId as string, { limit });
    ...
    }, [allTickets, search, date, activePriority, activeCategory]);
            ...
            render: (_value, ticket: Ticket) => (
                <div className="flex gap-4 items-center justify-start">
                    <Button
                        onClick={(e) => {
                            e.stopPropagation()
                            router.push(`/fleet/support/${ticket.id}`)
                        }}
                ...
                <Tabs defaultValue={activeTab} onValueChange={(value) => setActiveTab(value as TicketTab)} className="w-full py-4 gap-0">
                    ...
                    /> :
                        <GenericTable
                            ...
                            selectable={true}
                            hoverable={true}
                            onRowClick={(ticket) => router.push(`/fleet/support/${ticket.id}`)}
